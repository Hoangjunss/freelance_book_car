...
@Component({
  selector: 'app-tourism-statistics',
  standalone: true,
  imports: [CommonModule,FormsModule],
  templateUrl: './tourism-statistics.component.html',
  styleUrl: './tourism-statistics.component.css'
})
export class TourismStatisticsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getStatisticTodayTourism();
    this.getStatisticYesterdayTourism();
    this.getStatisticMonthTourism(new Date().getMonth(), new Date().getFullYear())
  }
  ...
  getStatisticTodayTourism(){
    this.statisticService.getStatisticTodayTourism().subscribe({
      next: (data) =>{
        if(data){
  ...
  getStatisticYesterdayTourism(){
    this.statisticService.getStatisticYesterdayTourism().subscribe({
      next: (data) =>{
        if(data){
  ...
  getStatisticMonthTourism(month:number, year:number){
    this.statisticService.getStatisticMonthTourism(month, year).subscribe({
      next: (data) =>{
        if(data){
